...
// 필터링된 데이터로 UI 업데이트
// function updateDisplay(filteredArr: any, isSelectFilter: boolean) {
//     if (isSelectFilter) {
...
// 메뉴 선택 이벤트 핸들러
// function handleMenuChange(data: any, selectedVal: string) {
...
// // 위험도 체크박스 이벤트 핸들러
// function handleRiskChange(data: any, checkedValues: any) {
...
// // 진행상태 체크박스 이벤트 핸들러
// function handleProgressChange(data: any, checkedValues: any) {
...
// function handleCheckboxChange(data: any, className: string, filterProperty: string) {
...
// 필터링 및 이벤트 리스너 초기화
// function initializeFilters(data: any) {
//     var select = document.querySelector('.status-select select');
//     select.addEventListener('change', (event) => handleMenuChange(data, event!.target!.value ));
...
export function drawTable(columns, rows) {
  ...
  COLUMNS.forEach(function (column) {
    ...
    const _colWidth = `width: ${column.width}`;
    _col.setAttribute("style", _colWidth);
  ...
  rows.forEach(function (row, idx) {
    ...
    COLUMNS.forEach(function (column) {
      ...
      if (column.child && column.child.length) {
        ...
        var _td = createCell(row, column.key, idx); // createCell 함수를 사용하여 셀 생성
        _trBody.appendChild(_td);
      }
    });
...
// create cell
function createCell(data, type, idx) {
  ...
  switch (type) {
    ...
    case "comments": // 비고
      _comment = createComment(data, type);
      _td.setAttribute("style", "text-align: left");
      _td.appendChild(_comment);
      return _td;
    case "requestStatus": // 수정 진행 상태
    case "isComplete": // 작업 진행 상태
      _comment = createProgress(data, type);
      _td.appendChild(_comment);
      return _td;
    // case 'priority': // 우선순위
