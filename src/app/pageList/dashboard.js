...
// modify info rendrer
// function drawModifyInfo(data: any) {
//     var wrapper = document.createDocumentFragment(); // fragment 생성
//     Object.entries(data).forEach(function ([key, value]) {
//         var item = document.createElement('td');
//         if (key === 'requestStatus' && data.requestDate !== '') {
//             let status:any = document.createElement('span');
//             let indicator = value === 0 ? 'pending' : value === 1 ? 'ongoing' : 'done';
//             let indicatorText = value === 0 ? '접수' : value === 1 ? '수정중' : '완료';
...
// draw status info
export function drawStatusInfo(arr) {
  ...
  statusItem = `
        ...
        `;
  // statusCont?.appendChild(statusItem);
  return statusItem;
}

// draw filters
// function drawFilters(data:any, target:any, type:any) {
//     // filter wrapper 지정
//     var wrapper = document.querySelector(target);

//     // 요소 생성
//     var filter = document.createElement(type);
//     let filterItem:any, filterContent:any;
 ...
 * isComplete - 0 : 진행대기 | 1 : 진행중 | 2 : 완료
 */
// function curStatus(dueToDate:any, isComplete:any) {
...
// function initUtilities(data:any) {
